...
for neu_j in np.arange(funa.n_neurons):
    ...
    try:
        astr = ''
        if aconn_ds_i is not None:
            astr="_aconn-ds-"+str(aconn_ds_i)
        if params['interacting']==0: astr = '-nonint'
        f = open('V0_3'+astr+'.json','r')
    ...
    folder = "/projects/LEIFER/francesco/simulations/activity_connectome_sign2"+astr+"/"
